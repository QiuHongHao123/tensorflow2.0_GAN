...
def trainStep_withGAN_G(low_img, G, D, g_optimizer):
    with tf.GradientTape() as g_tape:
        noise_img = G(low_img)
...
def train_withWGAN(train_database: tf.data.Dataset, epochs, batchsize, continue_train=False):
    ...
    checkpointManager = tf.train.CheckpointManager(checkpoint, directory='/content/drive/MyDrive/denoise_GAN/model',
                                                   max_to_keep=3)
    ...
    for epoch in range(epochs):
        ...
        for i, (low_img, full_img) in enumerate(train_database):
            # train step
            if i % 5 == 0:
                g_l += trainStep_withGAN_G(low_img, G, D, G_optimizer)
