...
train_images, test_db = loadmnist.mnist_dataset()
...
class Dense(layers.Layer):
    def __init__(self, input_dim, units):
        ...
        self.b = tf.Variable(initial_value=tf.zeros(shape=(1, units), dtype=tf.float32),
...
def train(train_images, train_labels, epochs):
    break_flag = 0
    index = list(range(tf.shape(train_images)))
